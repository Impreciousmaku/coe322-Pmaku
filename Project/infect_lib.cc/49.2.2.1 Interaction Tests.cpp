...
int main() {
    ...
    infectedPerson.infect(flu);

    for (int step = 1;; ++step) {
        joe.one_more_day();
        joe.infect(infectedPerson);
...
void Population::one_more_day() {
    ...
    // Simulate contacts and infection
    for (Person& person : people) {
        if (person.get_state() == Person::State::Sick) {
            // Sick person comes into contact with a number of other members
            for (int i = 0; i < person.get_contacts_made(); ++i) {
                int index = rand() % people.size();
                person.infect(people[index]);
            }
        } else {
            // Non-sick person makes contacts randomly
            int numContacts = rand() % 6; // Let's say a person makes up to 5 contacts
            for (int i = 0; i < numContacts; ++i) {
                int index = rand() % people.size();
                people[index].make_contact();
            }
        }
    }
}
